...
pygame.init()
# 顯示Title.
pygame.display.set_caption("Tetris Game")  # 英文標題
# 建立畫佈大小.
# 全螢幕模式.
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if not paused:
                # 變換方塊-上.
                if event.key == pygame.K_UP and game_mode == 0:
                    # 在右邊界不能旋轉.
                    if (container_x == 8):
                        break
                    # 判斷磚塊N1、N2、I.
                ...
                #-----------------------------------------------------------------
                # 直接降到底
                if event.key == pygame.K_SPACE and game_mode == 0:
                    # 持續往下直到不能再移動
                    while True:
                        container_y += 1
                        transformToBricks(brick_id, brick_state)
                        if not ifCopyToBricksArray():
                            container_y -= 1
                            transformToBricks(brick_id, brick_state)
                            copyToBricksArray()  # 直接複製到容器
                            # 判斷與設定要清除的方塊
                            lines = ifClearBrick() / 10
                            if (lines > 0):
                                lines_number = lines_number + lines
                                game_mode = 1
                            # 產生新方塊（只重設位置，不判斷 game_over）
                            container_x = 3
                            container_y = -4
                            brick_id = brick_next_id
                            brick_next_id = random.randint(1, 7)
    ...
    # 暫停時顯示暫停對話方塊於畫面中央，不處理遊戲邏輯
    if paused:
        # 半透明對話方塊
        dialog_width = 320
        dialog_height = 120
        dialog_x = (canvas_width - dialog_width) // 2
        dialog_y = (canvas_height - dialog_height) // 2
        dialog_surface = pygame.Surface((dialog_width, dialog_height), pygame.SRCALPHA)
        dialog_surface.fill((50, 50, 50, 180))  # 半透明深灰色

        # 將對話方塊畫到主畫布
        canvas.blit(dialog_surface, (dialog_x, dialog_y))

        # 顯示暫停文字 (改為英文)
        showFont("Game Paused", dialog_x + 80, dialog_y + 30, color_white)
        showFont("Press Z to Resume", dialog_x + 100, dialog_y + 70, color_gray_green)
    ...
    # 顯示訊息.
    showFont("Next Block", 588, 16, color_gray)

    showFont("Max Lines", 588, 190, color_gray)
    showFont(str(int(lines_number_max)), 588, 220, color_gray)

    showFont("Current Lines", 588, 260, color_gray)
